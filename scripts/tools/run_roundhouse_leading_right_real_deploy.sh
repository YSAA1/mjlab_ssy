...
RUN_NAME_PATTERN="${MJLAB_RUN_NAME_PATTERN:-*g1_mode15_roundhouse_leading_right*}"
SELECTED_RUN_DIR=""
...
LOG_ROOT="$WORKTREE/logs/roundhouse_leading_right_real_deploy"
CTRL_SESSION="roundhouse_leading_right_real_ctrl"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/tools/run_roundhouse_leading_right_real_deploy.sh prepare
  bash scripts/tools/run_roundhouse_leading_right_real_deploy.sh start <network-interface>
  bash scripts/tools/run_roundhouse_leading_right_real_deploy.sh stop
  bash scripts/tools/run_roundhouse_leading_right_real_deploy.sh status
...
Real-robot FSM:
  ...
  Mimic_RoundhouseLeadingRight -> [B] -> Passive
  Mimic_RoundhouseLeadingRight -> [RB + B] -> Velocity
  Mimic_RoundhouseLeadingRight ends -> Velocity
EOF
}
...
prepare_bundle() {
  ...
  mkdir -p "$ROUNDHOUSE_POLICY_DIR/exported" "$ROUNDHOUSE_POLICY_DIR/params"
  ...
  project_python - "$ACTIVE_CONFIG" "$ROUNDHOUSE_STATE" <<'PY'
from pathlib import Path
import sys
import yaml

path = Path(sys.argv[1])
roundhouse_state = sys.argv[2]
...
start_ctrl() {
  ...
  sha256sum "$ROUNDHOUSE_POLICY" "$ROUNDHOUSE_MOTION" "$ROUNDHOUSE_DEPLOY_YAML" > "$log_dir/hash_selected.txt"
