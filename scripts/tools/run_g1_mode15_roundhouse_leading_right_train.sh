...
TASK_ID="${MJLAB_TASK_ID:-Mjlab-Tracking-Flat-Unitree-G1-RoundhouseLeadingRight-No-State-Estimation}"
EXPERIMENT_NAME="${MJLAB_EXPERIMENT_NAME:-g1_tracking_roundhouse_leading_right_no_state}"
MOTION_FILE="${MJLAB_MOTION_FILE:-data/motions/g1_roundhouse_leading_right/mjlab/motion.npz}"
NUM_ENVS="${MJLAB_NUM_ENVS:-4096}"
MAX_ITERATIONS="${MJLAB_MAX_ITERATIONS:-5000}"
RUN_NAME_PREFIX="${MJLAB_RUN_NAME_PREFIX:-g1_mode15_roundhouse_leading_right_4096env_5000iter}"
RUN_NAME_PATTERN="${MJLAB_RUN_NAME_PATTERN:-*${RUN_NAME_PREFIX}*}"
SESSION="${MJLAB_TRAIN_SESSION:-g1_mode15_roundhouse_leading_right_5000iter}"
LOG_ROOT="$WORKTREE/logs/g1_mode15_roundhouse_leading_right_5000iter"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/tools/run_g1_mode15_roundhouse_leading_right_train.sh start
  bash scripts/tools/run_g1_mode15_roundhouse_leading_right_train.sh status
  bash scripts/tools/run_g1_mode15_roundhouse_leading_right_train.sh export [checkpoint.pt] [output.onnx]
  bash scripts/tools/run_g1_mode15_roundhouse_leading_right_train.sh finalize-deploy
  bash scripts/tools/run_g1_mode15_roundhouse_leading_right_train.sh stop

Defaults:
  task: Mjlab-Tracking-Flat-Unitree-G1-RoundhouseLeadingRight-No-State-Estimation
  motion: data/motions/g1_roundhouse_leading_right/mjlab/motion.npz
  iterations: 5000
EOF
}
...
start_train() {
  ...
  printf 'Started 5000-iter mode15 roundhouse training in tmux session: %s\n' "$SESSION"
  printf 'Run name: %s\n' "$run_name"
...
export_policy() {
  ...
  if [[ -z "$run_dir" ]]; then
    printf 'No roundhouse 5000-iter run found under logs/rsl_rl/%s.\n' "$EXPERIMENT_NAME" >&2
    exit 2
  fi
  ...
  output="${2:-$run_dir/roundhouse_leading_right_deploy_actor.onnx}"
  (
    cd "$WORKTREE"
...
finalize_deploy() {
  ...
  export_policy "$checkpoint" "$run_dir/roundhouse_leading_right_deploy_actor.onnx"
  bash "$WORKTREE/scripts/tools/run_roundhouse_leading_right_real_deploy.sh" prepare
